...
function countElementsByInterval(array, interval) {
  ...
  array.forEach((element) =>{
    ...
    // let key = Math.floor(element / interval) * interval + '~' + (Math.floor(element / interval) + 1) * interval;
          ...
          {list.map((datas, index) => { 
            ...
            return(<>
              <Slider
                  ...
                  />
                 <SurveyItem interval={intervals[index]}  title = {datas.title} subtitle = {datas.subtitles.subtitle[required_Language]} hist = {hist} /></>
          )})}
        </Stack>
      </Scrollbar>
...
AppSurveys.propTypes = {
  ...
  subtitles: PropTypes.any.isRequired,
};

// ----------------------------------------------------------------------

function SurveyItem({ title,subtitle, hist , interval}) {
  console.log(subtitle)
  console.log(Object.keys(hist))
